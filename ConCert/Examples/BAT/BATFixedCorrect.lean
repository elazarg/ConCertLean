/- Functional-correctness lemmas for examples/bat/BATFixedCorrect.v. -/
...
private theorem fixed_token_receive_ok
    ...
    (h : Fixed.receive chain ctx prev_state (some (.tokenMsg msg)) =
  ...
  unfold Fixed.receive receive_token at h
  cases hfin : (!prev_state.isFinalized)
  · simp [hfin] at h
    cases htok :
        ConCert.Examples.EIP20.EIP20Token.receive chain ctx prev_state.token_state
          (some msg) with
...
theorem try_transfer_balance_correct
    ...
    (h : Fixed.receive chain ctx prev_state (some (transfer to_ amount)) =
      .Ok (new_state, new_acts)) :
    ConCert.Examples.EIP20.EIP20Token.transfer_balance_update_correct
      prev_state.token_state new_state.token_state ctx.ctx_from to_ amount =
        true := by
  rcases fixed_token_receive_ok (Base := Base) h with
...
theorem try_transfer_is_some
    ...
    (ctx : @ContractCallContext Base) (to_ : Base.Address)
    (amount : TokenValue) (hfinalized : state.isFinalized = true) :
    (ctx.ctx_amount ≤ 0 ∧
        amount ≤ with_default 0 (FMap.find ctx.ctx_from (balances state))) ↔
      isOk (Fixed.receive chain ctx state (some (transfer to_ amount))) =
        true := by
  constructor
  · intro h
    have hok :=
    ...
    unfold Fixed.receive receive_token
    simp [transfer, hfinalized]
    cases htok :
...
theorem try_transfer_from_balance_correct
    ...
    {amount : TokenValue} {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state
  ...
  · intro h
    have hok :
        isOk
          ...
          true := by
      unfold Fixed.receive receive_token at h
      simp [transfer_from, hfinalized] at h
      cases htok :
          ConCert.Examples.EIP20.EIP20Token.receive chain ctx state.token_state
            (some
              (ConCert.Examples.EIP20.EIP20Token.Msg.transfer_from from_ to_ amount)) <;>
...
theorem try_approve_allowance_correct
    ...
    {amount : TokenValue} {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some (approve delegate amount)) =
      .Ok (new_state, new_acts)) :
...
theorem try_approve_is_some
    ...
    (ctx : @ContractCallContext Base) (delegate : Base.Address)
    (amount : TokenValue) :
    ctx.ctx_amount ≤ 0 ∧ state.isFinalized = true ↔
      isOk (Fixed.receive chain ctx state (some (approve delegate amount))) =
        true := by
  by_cases hfinalized : state.isFinalized = true
  · constructor
    · intro h
      have hok :=
        (ConCert.Examples.EIP20.EIP20Token.try_approve_is_some
          state.token_state chain ctx delegate amount).mp h.1
      unfold Fixed.receive receive_token
      simp [approve, hfinalized]
      cases htok :
          ConCert.Examples.EIP20.EIP20Token.receive chain ctx state.token_state
  ...
  · have hnotFinalized : state.isFinalized = false := by
      cases hs : state.isFinalized
      · rfl
      · exfalso
        exact hfinalized (by simp [hs])
    constructor
    · intro h
      exact (hfinalized h.2).elim
    · intro h
      simp [Fixed.receive, approve, hnotFinalized, isOk] at h
...
private theorem fixed_create_tokens_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some .create_tokens) =
      .Ok (new_state, new_acts)) :
    try_create_tokens true ctx.ctx_from ctx.ctx_amount chain.current_slot
        prev_state = .Ok new_state ∧
      new_acts = [] := by
  unfold Fixed.receive receive_bat_original at h
...
private theorem try_create_tokens_ok_shape
    ...
    (h : try_create_tokens true sender payload slot state = .Ok new_state) :
    0 < payload ∧
      new_state =
        { state with
          token_state :=
  ...
  by_cases hbad :
      (((state.isFinalized = true ∨ slot < state.fundingStart) ∨
        ...
        Base.address_eqb sender state.batFundDeposit = true)
  ·
      simp [hbad] at h
  ·
...
private theorem try_create_tokens_ok_requirements
    ...
    0 < payload ∧
      ...
      sender ≠ state.batFundDeposit := by
  unfold try_create_tokens at h
  by_cases hbad :
      (((state.isFinalized = true ∨ slot < state.fundingStart) ∨
          state.fundingEnd < slot) ∨
  ...
  · by_cases hpayload : payload <= 0
    ...
    · let tokens := payload.toNat * state.tokenExchangeRate
      by_cases hcap : state.tokenCreationCap < total_supply state + tokens
      · simp [hbad, hpayload, tokens, hcap] at h
      · refine ⟨lt_of_not_ge hpayload, ?_, ?_, ?_, ?_, ?_⟩
        · cases hfin : state.isFinalized
          · rfl
          · exfalso
            exact hbad (Or.inl (Or.inl (Or.inl hfin)))
        · exact le_of_not_gt
            (fun hlt => hbad (Or.inl (Or.inl (Or.inr hlt))))
        · exact le_of_not_gt
            (fun hlt => hbad (Or.inl (Or.inr hlt)))
        · exact le_of_not_gt (by simpa [tokens] using hcap)
        · intro heq
          exact hbad (Or.inr ((Base.address_eqb_spec _ _).mpr heq))
...
theorem try_create_tokens_balance_correct
  ...
  rcases fixed_create_tokens_ok (Base := Base) h with ⟨hraw, _⟩
...
theorem try_create_tokens_acts_correct
    ...
    (h : Fixed.receive chain ctx prev_state (some .create_tokens) =
      .Ok (new_state, new_acts)) :
    new_acts = [] :=
  (fixed_create_tokens_ok (Base := Base) h).2
...
theorem try_create_tokens_is_some
    ...
    (0 < ctx.ctx_amount ∧
        ...
        total_supply state +
          ...
          state.tokenCreationCap ∧
        ctx.ctx_from ≠ state.batFundDeposit) ↔
      ∃ new_state new_acts,
        Fixed.receive chain ctx state (some .create_tokens) =
          .Ok (new_state, new_acts) := by
  constructor
  · intro h
    rcases h with
      ⟨hAmount, hFinalized, hStart, hEnd, hCap, hSender⟩
    ...
    have hbad :
        ¬(((state.isFinalized = true ∨
          ...
          Base.address_eqb ctx.ctx_from state.batFundDeposit = true) := by
      rintro (((hfin | hlt) | hlt) | heq)
      · rw [hFinalized] at hfin
        cases hfin
      · exact (not_lt.mpr hStart) hlt
      · exact (not_lt.mpr hEnd) hlt
      · exact hSender ((Base.address_eqb_spec _ _).mp heq)
    have hpayload : ¬ ctx.ctx_amount ≤ 0 := not_le.mpr hAmount
    have hcap : ¬ state.tokenCreationCap < total_supply state + tokens :=
      not_lt.mpr hCap
    unfold Fixed.receive receive_bat_original try_create_tokens
  ...
  · rintro ⟨new_state, new_acts, h⟩
    exact try_create_tokens_ok_requirements (Base := Base)
      (fixed_create_tokens_ok (Base := Base) h).1

private theorem fixed_finalize_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some .finalize) =
      .Ok (new_state, new_acts)) :
    try_finalize false ctx.ctx_from chain.current_slot ctx.ctx_contract_balance
...
private theorem try_finalize_ok_shape
    ...
    {new_acts : List (@ActionBody Base)}
    (h : try_finalize false sender slot contract_balance state =
      .Ok (new_state, new_acts)) :
    new_state = { state with isFinalized := true } ∧
...
theorem try_finalize_isFinalized_correct
  ...
  have hraw := fixed_finalize_ok (Base := Base) h
...
theorem try_finalize_only_change_isFinalized
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some .finalize) =
      .Ok (new_state, new_acts)) :
    { prev_state with isFinalized := new_state.isFinalized } = new_state := by
  have hraw := fixed_finalize_ok (Base := Base) h
  rw [(try_finalize_ok_shape (Base := Base) hraw).1]

theorem try_finalize_preserves_total_supply
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some .finalize) =
...
theorem try_finalize_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      ∃ new_state new_acts,
        Fixed.receive chain ctx state (some .finalize) =
          .Ok (new_state, new_acts) := by
  constructor
  · intro h
    rcases h with
      ⟨hAmount, hFinalized, hSender, hMin, hDone⟩
    let new_state := { state with isFinalized := true }
    refine ⟨new_state,
    ...
    unfold Fixed.receive receive_bat_original try_finalize
    simp [hamount, hbad, hearly, new_state]
  · rintro ⟨new_state, new_acts, h⟩
    have hraw := fixed_finalize_ok (Base := Base) h
...
private theorem fixed_refund_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state (some .refund) =
      .Ok (new_state, new_acts)) :
    try_refund_original true ctx.ctx_from chain.current_slot prev_state =
      .Ok (new_state, new_acts) := by
  unfold Fixed.receive receive_bat_original at h
  by_cases hamount : ctx.ctx_amount > 0
  · simp [hamount] at h
...
private theorem try_refund_ok_shape
    ...
    (h : try_refund_original true sender slot state =
      .Ok (new_state, new_acts)) :
    ∃ sender_bats,
  ...
  · rename_i hnotBad
    ...
    | some sender_bats =>
        ...
        · simp [hfind, hzero] at h
          ...
          · intro heq
            apply hnotBad
            simp [(Base.address_eqb_spec _ _).mpr heq]
          · have hne : sender_bats ≠ 0 := by
              intro hz
              subst hz
              simp at hzero
...
theorem try_refund_balance_correct
    ...
    (h : Fixed.receive chain ctx prev_state (some .refund) =
      .Ok (new_state, new_acts)) :
    with_default 0 (FMap.find ctx.ctx_from (balances new_state)) = 0 := by
  have hraw := fixed_refund_ok (Base := Base) h
...
theorem try_refund_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      ∃ new_state new_acts,
        Fixed.receive chain ctx state (some .refund) =
          .Ok (new_state, new_acts) := by
  constructor
  · intro h
    rcases h with
      ⟨hAmount, hFinalized, hEnd, hMin, hSender, hBalance⟩
    have hamount : ¬ ctx.ctx_amount > 0 := not_lt.mpr hAmount
    have hbad :
        ¬(state.isFinalized ||
            decide (chain.current_slot ≤ state.fundingEnd) ||
            decide (state.tokenCreationMin ≤ total_supply state) ||
            (true &&
              Base.address_eqb ctx.ctx_from state.batFundDeposit)) = true := by
      intro hbad
      rw [Bool.or_eq_true, Bool.or_eq_true, Bool.or_eq_true] at hbad
      rcases hbad with ((hfin | hend) | hmin) | hsender
      · rw [hFinalized] at hfin
        cases hfin
      ...
      · have heqb :
            Base.address_eqb ctx.ctx_from state.batFundDeposit = true := by
          simpa using hsender
        exact hSender ((Base.address_eqb_spec _ _).mp heqb)
    ...
    | some sender_bats =>
        ...
        let new_token_state : @ConCert.Examples.EIP20.EIP20Token.State Base :=
          { total_supply := total_supply state - sender_bats,
            balances := FMap.add ctx.ctx_from 0 (balances state),
            allowances := allowances state }
        let new_state := { state with token_state := new_token_state }
        refine ⟨new_state,
          [.act_transfer ctx.ctx_from
        ...
        have hSenderFalse :
            Base.address_eqb ctx.ctx_from state.batFundDeposit = false :=
          Address.address_eq_ne ctx.ctx_from state.batFundDeposit hSender
        unfold Fixed.receive receive_bat_original try_refund_original
        simp [hamount, hfind, hzero, new_token_state, new_state,
          hFinalized, hEnd, hMin, hSenderFalse]
  · rintro ⟨new_state, new_acts, h⟩
    have hraw := fixed_refund_ok (Base := Base) h
    ...
    exact ⟨hamount, hreq.1, hreq.2.1, hreq.2.2.1,
      hreq.2.2.2.1, hreq.2.2.2.2⟩

private theorem fixed_init_ok_shape
    {state : @State Base} {chain : Chain} {ctx : @ContractCallContext Base}
    {setup : @Setup Base}
    (h : Fixed.init chain ctx setup = .Ok state) :
    state =
      mk_state setup
        (base_token_state setup.batFund
          (FMap.add setup.batFundDeposit_ setup.batFund FMap.empty)) := by
  unfold Fixed.init init_fixed init_original at h
  by_cases hvalid : valid_fixed_setup chain ctx setup true
  · simp [hvalid] at h
  · simp [hvalid] at h
    exact h.symm

theorem init_bat_balance_correct
    {state : @State Base} {chain : Chain} {ctx : @ContractCallContext Base}
    {setup : @Setup Base}
    (h : Fixed.init chain ctx setup = .Ok state) :
    with_default 0 (FMap.find state.batFundDeposit (balances state)) =
      setup.batFund := by
  rw [fixed_init_ok_shape (Base := Base) h]
  simp [mk_state, base_token_state, balances, FMap.find_add, with_default,
    withDefault]
...
theorem init_other_balances_correct
  ...
  have hshape := fixed_init_ok_shape (Base := Base) h
  subst hshape
  simp [mk_state, base_token_state, balances, with_default, withDefault,
    FMap.find_add_ne setup.batFundDeposit_ account _ _ (Ne.symm h_account),
    FMap.find_empty]
...
theorem init_total_supply_correct
    ...
    {setup : @Setup Base}
    (h : Fixed.init chain ctx setup = .Ok state) :
    total_supply state = setup.batFund := by
  rw [fixed_init_ok_shape (Base := Base) h]
  rfl
...
theorem try_refund_update_balances_sum
    ...
    sum_balances prev_state =
      sum_balances new_state +
        with_default 0 (FMap.find ctx.ctx_from (balances prev_state)) := by
  have hraw := fixed_refund_ok (Base := Base) h
  rcases try_refund_ok_shape (Base := Base) hraw with
    ⟨sender_bats, hfind, hne, hstate, hacts⟩
  subst hstate
  ...
  change
    ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum (balances prev_state) =
      ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum
          (FMap.add ctx.ctx_from 0 (balances prev_state)) +
        with_default 0 (FMap.find ctx.ctx_from (balances prev_state))
  rw [ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum_add_existing
    ctx.ctx_from sender_bats 0 (balances prev_state) hfind]
  rw [hfind]
  simp [with_default, withDefault]
  have hfind' :
      FMap.find ctx.ctx_from prev_state.token_state.balances =
...
theorem init_preserves_balances_sum
  ...
  change
    ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum
      ...
      setup.batFund
  rw [ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum_add_new]
  · simp [ConCert.Examples.EIP20.EIP20Token.fmap_nat_sum, FMap.elements_empty]
  · exact FMap.find_empty setup.batFundDeposit_

theorem receive_preserves_constants
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base} {msg : Option Msg}
    {new_acts : List (@ActionBody Base)}
    (h : Fixed.receive chain ctx prev_state msg = .Ok (new_state, new_acts)) :
  ...
  | some msg =>
      ...
      | create_tokens =>
          ...
          simp
      | finalize =>
          have hchange := try_finalize_only_change_isFinalized
            (Base := Base) h
          rw [← hchange]
          simp
      | refund =>
...
theorem receive_total_supply_increasing
  ...
  | some msg =>
      ...
      | finalize =>
          exact le_of_eq
            (try_finalize_preserves_total_supply (Base := Base) h)
      | refund =>
          have hreq :=
            (try_refund_is_some (Base := Base) prev_state chain ctx).mpr
              ⟨new_state, new_acts, h⟩
          rcases hreq with
            ⟨_, _, hafterEnd, hbelowMin, _, _⟩
