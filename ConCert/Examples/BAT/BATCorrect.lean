/- Functional-correctness lemmas for examples/bat/BATCorrect.v. -/
...
private theorem original_token_receive_ok
    ...
    (h : Original.receive chain ctx prev_state (some (.tokenMsg msg)) =
  ...
  unfold Original.receive receive_token at h
  cases htok :
      ConCert.Examples.EIP20.EIP20Token.receive chain ctx prev_state.token_state
        (some msg) with
  | Err e =>
...
theorem try_transfer_balance_correct
    ...
    (h : Original.receive chain ctx prev_state (some (transfer to_ amount)) =
      .Ok (new_state, new_acts)) :
    ConCert.Examples.EIP20.EIP20Token.transfer_balance_update_correct
      prev_state.token_state new_state.token_state ctx.ctx_from to_ amount =
        true := by
  rcases original_token_receive_ok (Base := Base) h with
...
theorem try_transfer_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      isOk (Original.receive chain ctx state (some (transfer to_ amount))) =
        true := by
  constructor
  · intro h
    have hok :=
    ...
    unfold Original.receive receive_token
    simp [transfer]
    cases htok :
...
theorem try_transfer_from_balance_correct
    ...
    {amount : TokenValue} {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state
...
theorem try_transfer_from_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      isOk
        (Original.receive chain ctx state
...
theorem try_approve_allowance_correct
    ...
    {amount : TokenValue} {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state (some (approve delegate amount)) =
      .Ok (new_state, new_acts)) :
...
theorem try_approve_is_some
    ...
    (amount : TokenValue) :
    ctx.ctx_amount ≤ 0 ↔
      isOk (Original.receive chain ctx state (some (approve delegate amount))) =
        true := by
  constructor
  · intro h
    have hok :=
...
private theorem original_create_tokens_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state (some .create_tokens) =
      .Ok (new_state, new_acts)) :
    try_create_tokens false ctx.ctx_from ctx.ctx_amount chain.current_slot
        prev_state = .Ok new_state ∧
      new_acts = [] := by
  unfold Original.receive receive_bat_original at h
...
theorem try_create_tokens_balance_correct
  ...
  rcases original_create_tokens_ok (Base := Base) h with ⟨hraw, _⟩
...
theorem try_create_tokens_acts_correct
    ...
    (h : Original.receive chain ctx prev_state (some .create_tokens) =
      .Ok (new_state, new_acts)) :
    new_acts = [] :=
  (original_create_tokens_ok (Base := Base) h).2
...
theorem try_create_tokens_is_some
    ...
    (0 < ctx.ctx_amount ∧
      ...
      ∃ new_state new_acts,
        Original.receive chain ctx state (some .create_tokens) =
  ...
  · intro h
    ...
    have hcap : ¬ state.tokenCreationCap < total_supply state + tokens :=
      not_lt.mpr hCap
    unfold Original.receive receive_bat_original try_create_tokens
  ...
  · rintro ⟨new_state, new_acts, h⟩
    exact try_create_tokens_ok_requirements (Base := Base)
      (original_create_tokens_ok (Base := Base) h).1

private theorem original_finalize_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state (some .finalize) =
      .Ok (new_state, new_acts)) :
...
theorem try_finalize_isFinalized_correct
  ...
  have hraw := original_finalize_ok (Base := Base) h
...
theorem try_finalize_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      ∃ new_state new_acts,
        Original.receive chain ctx state (some .finalize) =
          .Ok (new_state, new_acts) := by
  constructor
  · intro h
    rcases h with
    ...
    unfold Original.receive receive_bat_original try_finalize
...
private theorem original_refund_ok
    {prev_state new_state : @State Base} {chain : Chain}
    {ctx : @ContractCallContext Base}
    {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state (some .refund) =
      .Ok (new_state, new_acts)) :
    try_refund_original false ctx.ctx_from chain.current_slot prev_state =
      .Ok (new_state, new_acts) := by
  unfold Original.receive receive_bat_original at h
  by_cases hamount : ctx.ctx_amount > 0
  · simp [hamount] at h
...
private theorem try_refund_ok_shape
    ...
    (h : try_refund_original false sender slot state =
      .Ok (new_state, new_acts)) :
    ∃ sender_bats,
...
theorem try_refund_balance_correct
  ...
  have hraw := original_refund_ok (Base := Base) h
...
theorem try_refund_is_some
    ...
    (ctx.ctx_amount ≤ 0 ∧
      ...
      ∃ new_state new_acts,
        Original.receive chain ctx state (some .refund) =
          .Ok (new_state, new_acts) := by
  constructor
  · intro h
    rcases h with
    ...
    | some sender_bats =>
        ...
        unfold Original.receive receive_bat_original try_refund_original
...
theorem init_bat_balance_correct
    ...
    with_default 0 (FMap.find state.batFundDeposit (balances state)) =
      setup.batFund := by
  rw [original_init_ok_shape (Base := Base) h]
...
theorem init_other_balances_correct
  ...
  have hshape := original_init_ok_shape (Base := Base) h
  subst hshape
...
theorem receive_preserves_constants
    ...
    {new_acts : List (@ActionBody Base)}
    (h : Original.receive chain ctx prev_state msg = .Ok (new_state, new_acts)) :
