/- Port of examples/fa1_2/FA1_2.v. -/
...
def non_zero_amount (amt : Amount) : Bool :=
  decide (0 < amt)
...
inductive Msg where
  | transfer (param : @TransferParam Base)
  | approve (param : @ApproveParam Base)
  | getAllowance (param : @GetAllowanceParam Base)
  | getBalance (param : @GetBalanceParam Base)
  | getTotalSupply (param : @GetTotalSupplyParam Base)
  deriving Serializable
...
def empty_allowance : FMap (Base.Address × Base.Address) Nat :=
  FMap.empty
...
def receive_allowance_ (n : Nat) : FA12ReceiverMsg Unit :=
  .receive_allowance n

def receive_balance_of_ (n : Nat) : FA12ReceiverMsg Unit :=
  .receive_balance_of n

def receive_total_supply_ (n : Nat) : FA12ReceiverMsg Unit :=
  .receive_total_supply n
...
def sum_balances (state : @State Base) : Nat :=
  ((FMap.elements state.tokens).map (fun p : Base.Address × Nat => p.2)).sum
