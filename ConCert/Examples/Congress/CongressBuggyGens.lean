/- Port of examples/congress/tests/Congress_BuggyGens.v over the local-chain backend. -/
...
namespace ConCert.Examples.Congress.BuggyGens
...
def congress_call (caller : TestAddress) (amount : Amount)
    (msg : @Buggy.Msg TestBase) : TestAction :=
  { act_origin := caller,
    act_from := caller,
    act_body := .act_call congress_addr amount (serialize msg) }

def initial_buggy_header : @BlockHeader TestBase :=
  nextBlockHeader (lc_initial AddrSize) creator_addr 100

def deploy_buggy_congress_action : TestAction :=
  { act_origin := creator_addr,
    act_from := creator_addr,
    act_body := create_deployment 50 (Buggy.contract (Base := TestBase)) setup }

def deploy_exploit_action : TestAction :=
  { act_origin := creator_addr,
    act_from := creator_addr,
    act_body := create_deployment 0
      (Buggy.exploit_contract (Base := TestBase)) () }

def initial_buggy_actions : List TestAction :=
  [ transferAction creator_addr member_addr 10,
    transferAction creator_addr candidate_addr 10,
    deploy_buggy_congress_action,
    deploy_exploit_action ]
...
def congress_deploy_result : Result TestLocalChainBuilder TestAddBlockError :=
  add_block AddrSize DepthFirst (lcb_initial AddrSize)
    initial_buggy_header initial_buggy_actions
...
def generated_call (env : @Environment TestBase) (caller : TestAddress)
    (msg : @Buggy.Msg TestBase) : GOpt TestAction := (do
...
def vote_proposal (env : @Environment TestBase)
    (state : @State TestBase) (vote : ProposalId → @Buggy.Msg TestBase) :
    GOpt TestAction := (do
...
def gBaseCongressAction (env : @Environment TestBase)
    (state : @State TestBase) : GOpt TestAction := (do
  let choice ← chooseNatBetween 0 9 (by decide)
  match choice with
  | 0 =>
  ...
  | 4 =>
      vote_proposal env state Buggy.Msg.vote_for_proposal
  | 5 =>
      vote_proposal env state Buggy.Msg.vote_against_proposal
  | 6 =>
  ...
  | _ =>
      ...
      | none => return none
      | some (pid, _) =>
          generated_call env congress_addr (.finish_proposal_remove pid) :
    G (Option TestAction))
...
def gCongressChainBuilder (length : Nat) (fuel : Nat := 2) :
  ...
  | .Err _ => genFailure "initial buggy Congress chain deployment failed"

end ConCert.Examples.Congress.BuggyGens
